...
## What you'll build
**In plain words:** your board becomes its own WiFi network. Your partner connects their laptop or
phone to it — no router, no room WiFi involved — opens a web address, and gets your live sensor
readings back as data. They can also send a setting *to* your board the same way, and your board
obeys it.
...
- **Serve HTTP endpoints** — a `GET` that returns live data as JSON, and a `POST` that accepts a setting and applies it.
...
4. **Routes and methods** — the same path with `GET` and `POST` are two different handlers.
...
- **`GET /api/reading`** returns live temperature, humidity and uptime as JSON.
- **`POST /api/config`** accepts a JSON body with a threshold and applies it to the device.
- The OLED shows the AP name, the board's IP, the current threshold and how many requests it has served.
- **The sensor, display and alarm keep running at full rate** while requests are being served.
- A malformed body, a wrong method or an unknown path returns a proper error code — never a crash, never a hang.
...
| Behaviour | Board starts an AP at boot. `GET /api/reading` returns temp/humidity/uptime as JSON. `POST /api/config` sets the alarm threshold. OLED shows SSID, IP, threshold and request count. Sensing and alarm run continuously regardless of traffic. |
...
| Connectivity | **Access-point mode only** — the board creates the network. Fixed IP `192.168.4.1`. HTTP server on port 80. No internet, no router, no cloud. |
...
| Safety | Sensing and indication only — nothing moves or heats. **Must never happen:** an inbound request changing device behaviour without validation, or HTTP traffic delaying the sensing and alarm path. **Safe state on boot:** AP up, alarm armed on the local knob threshold, before any client can connect. A `POST` value outside the accepted range is rejected with `400` and the previous threshold is retained. |
| Failure modes | Malformed or non-JSON body → `400`, threshold unchanged. Unknown path → `404`. Sensor read failure → serve the last good value flagged as stale, never a fabricated zero. No clients connected is normal, not an error. |
...
| Out of scope | No station mode, no internet, no HTTPS, no authentication beyond the AP password, no serving HTML pages — JSON only. |
| Acceptance | Partner joins the AP and gets valid JSON from `GET /api/reading`; a valid `POST /api/config` visibly changes the threshold on the OLED; an out-of-range or malformed POST is rejected and changes nothing; an unknown path returns 404; display and alarm keep running while the endpoints are being hammered. |
...
- **Stage 2 — Serve a reading (25 min):** add `GET /api/reading` returning JSON. **Test:** partner opens `http://192.168.4.1/api/reading` in a browser and sees live values; breathe on your sensor and refresh — the numbers move. Save a **http-server-get** template.
- **Stage 3 — Accept a setting (25 min):** add `POST /api/config` that parses a JSON body and applies the threshold. **Test:** partner posts a new threshold with `curl` and the OLED changes. Save a **http-server-post** template.
...
- **Stage 5 — Hostile clients (15 min):** partner sends a body that isn't JSON, a wildly out-of-range threshold, a `GET` to the POST path, and a request to `/nonsense`. **Test:** each returns a sensible code and the device carries on unchanged.
...
- ⚠ **No validation on the POST.** It will apply whatever number arrives, including nonsense. Range-check it and answer `400` when it fails.
...
- [ ] Partner's `POST /api/config` **visibly changes the threshold** on your display.
