...
| Safety | **A servo moves a physical arm — this is the real risk in this build.** **Must never happen:** the servo commanded beyond its travel limits, driven into a mechanical end-stop (a stalled servo draws heavy current continuously and can brown out the board), or moved by a frame that failed its checksum or address check. Enforce the 500–2500 µs limits in software and reject invalid frames before they reach the actuator. **Safe state:** park at a defined angle on boot *before* the bus is read, and again on 3 missed frames or bus loss. Motion must be non-blocking so a long move cannot delay fault detection. |
