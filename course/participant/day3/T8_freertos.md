...
| Safety | Concurrency adds a new hazard path: an undersized stack reboots the board **mid-motion**. **Must never happen:** an actuator resuming or holding a stale commanded position after an unplanned reset, a task driving an actuator before the system has reached a known state, or the watchdog being disabled to silence a symptom. **Safe state:** any actuator is commanded to its park position during startup, before any task is permitted to drive it. A triggered watchdog is a design fault to fix, not a message to suppress. |
