...
| Safety | **Required whatever you pick — state it as a specification, not a precaution.** If your project moves a servo or drives a latch: name the park position for boot, fault *and* power loss; state the software travel limits; state that only validated, range-checked commands may reach the actuator. If it is sensing/indication only, say so and state what must never happen anyway (e.g. an alarm suppressed by a network fault). Anything running unattended needs a defined timeout or watchdog. |
