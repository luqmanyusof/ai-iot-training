...
|---|---|---|---|---|---|
| OLED SSD1315 | I2C | 0x3C | **Grove Port 2 (D21/D22) — T3, T6, T7** | U8g2 **or** Adafruit_SSD1306 + Adafruit_GFX | SSD1315 is SSD1306-compatible; 3.3/5V ok |
...
| Gesture PAJ7660 | I2C (or SPI) | **0x73 — VERIFY by scan** | **Grove Port 2 (D21/D22) — T4, T5, T8, T9** | Seeed PAJ7660 lib | 2nd of only two I2C devices; 15+ gestures, **5–40 cm** range |
...
1. **Only ONE I²C Grove port exists — Port 2 (D21/D22).** The kit has two I²C devices (OLED `0x3C`, Gesture `0x73`), so they cannot both use a Grove port. **They are never needed at the same time:** OLED serves T3, T6 and T7; gesture takes Port 2 for T4, T5, T8 and T9 (those projects run headless, reporting over serial, NeoPixel or their dashboard). The Maker/QWIIC port shares the same D21/D22 pins but is a **QWIIC/Stemma connector — a Grove module needs a conversion cable to use it**. An I²C Hub or that cable is only required if you want both devices live simultaneously. DHT11 is digital, not on the bus.
...
5. **Gesture PAJ7660** — I2C on **Grove Port 2** for T4, T5, T8 and T9 (the OLED comes off; those projects run headless). Detection range **5–40 cm**; avoid a highly reflective or busy background behind the hand. Address `0x73` is **not definitive in the vendor docs — scan the bus.**
...
| Port | Assigned to |
|---|---|
| I2C Grove Port 2 (D21/D22) | **OLED** (T3, T6, T7) ⇄ **Gesture PAJ7660** (T4, T5, T8, T9) — one port, swapped per project |
...
| I²C device missing on scan | wrong port, or the other device is fitted | **only Port 2 is I²C**; OLED T3/T6/T7, gesture T4/T5/T8/T9 |
