...
| Crowtail DHT11 | digital 1-wire | — | any digital Grove port | DHT sensor library | slow (~1 Hz); Grove-compatible connector; poll every 1–2 s |
...
1. **Only ONE I²C Grove port exists — Port 2 (D21/D22).** The kit has two I²C devices (OLED `0x3C`, Gesture `0x73`), so they cannot both use a Grove port. **They are never needed at the same time:** OLED runs T3–T7, gesture takes Port 2 from T8 onward (device goes headless — the T6 dashboard is the display). The Maker/QWIIC port shares the same D21/D22 pins but is a **QWIIC/Stemma connector — a Grove module needs a conversion cable to use it**. An I²C Hub or that cable is only required if you want both devices live simultaneously. DHT11 is digital, not on the bus.
2. **Rotary Angle on ADC1 only** (D32/D33). Wiring it to ADC2 (D25/D26/D32-no/D33-no... use D32/D33 = ADC1) works until WiFi comes up on Day 2, then reads garbage — looks like a code bug.
